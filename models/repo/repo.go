...
func ComposeSSHCloneURL(doer *user_model.User, ownerName, repoName string, groupPath string) string {
	...
	if setting.SSH.Port != 22 {
		...
		return fmt.Sprintf("ssh://%s@%s/%s/%s%s.git", sshUser, sshHost, url.PathEscape(ownerName), groupSegmentWithTrailingSlash(groupPath), url.PathEscape(repoName))
	}
	...
	return fmt.Sprintf("%s@%s:%s/%s%s.git", sshUser, sshHost, url.PathEscape(ownerName), groupSegmentWithTrailingSlash(groupPath), url.PathEscape(repoName))
}
