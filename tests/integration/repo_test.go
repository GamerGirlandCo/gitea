...
func TestGenerateRepository(t *testing.T) {
	...
	exist, err := util.IsExist(repo_model.RepoPath(user2.Name, generatedRepo.Name, generatedRepo.
		GroupPath()))
	require.NoError(t, err)
	...
	exist, err = util.IsExist(repo_model.RepoPath(user2.Name, generatedRepo.Name, generatedRepo.GroupPath()))
	assert.NoError(t, err)
