...
func TestCreateRepositoryDirectly(t *testing.T) {
	...
	exist, err := util.IsExist(repo_model.RepoPath(user2.Name, createdRepo.Name, createdRepo.GroupPath())
	assert.NoError(t, err)
	assert.True(t, exist)
	...
	assert.NoError(t, os.MkdirAll(repo_model.RepoPath(user2.Name, createdRepo.Name, createdRepo.GroupPath(), os.ModePerm))
